  ...
  protected onSubmit(): void {
    ...
    this.dialog
      .open(CastedVotesConfirmationComponent, {
        ...
        autoFocus: true,
        disableClose: true,
        data: { castedVotes: this.castedVotes },
      })
      .afterClosed()
      .subscribe((confirm) => {
        if (confirm === true) {
          ...
          this.ballotService
            .post({
              content: JSON.stringify(this.castedVotes),
            })
            .subscribe((response) => {
  ...
  protected onModelChange(): void {
    this.submitBtnIsDisabled.update(() => {
      ...
      Object.keys(this.model).forEach((k) => {
        this.castedVotes[k] = Object.keys(this.model[k])
      ...
      const hasNoSelectedCandidates = Object.keys(this.castedVotes).every(
        (k) => this.castedVotes[k].length === 0
      );
